...
namespace LZebra {
	...
	template<typename T>
	inline void RowVector<T>::insert(size_t index, size_t s, const T val)
	{
		auto& mat = Matrix<T>::matrix;
	...
	template<typename T>
	inline void RowVector<T>::shed(size_t i1, size_t i2)
	{
		auto& mat = Matrix<T>::matrix;
	...
	template<typename T>
	inline void RowVector<T>::shed_head(size_t i)
	{
		this->shed(0, i);
	}

	template<typename T>
	inline void RowVector<T>::shed_tail(size_t i)
	{
	...
	template<typename T>
	inline void RowVector<T>::operator=(const SubView<T>& V)
	{
		Matrix<T>::operator= (V.n_rows == 1 ? Matrix<T>(V) : tenseopr::vectorise(Matrix<T>(V), 1));
	}
	...
	template<typename T>
	inline RowVector<T>::RowVector(const SubView<T>& V) : Matrix<T>(V.n_rows == 1 ? Matrix<T>(V) : tenseopr::vectorise(Matrix<T>(V), 1))
	{
