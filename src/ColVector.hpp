...
namespace LZebra {
	...
	template<typename T>
	inline void ColVector<T>::operator=(const SubView<T>& V)
	{
		Matrix<T>::operator= (V.n_cols == 1 ? Matrix<T>(V) : tenseopr::vectorise(Matrix<T>(V), 0));
	}
	...
	template<typename T>
	inline ColVector<T>::ColVector(const SubView<T>& V) : Matrix<T>(V.n_cols == 1 ? Matrix<T>(V) : tenseopr::vectorise(Matrix<T>(V), 0))
	{
