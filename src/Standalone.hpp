...
namespace LZebra{
	...
	namespace tenseopr
	{
		...
		template<typename C, typename T>
		Matrix<std::complex<C>> conv_to(cpmat m)
		{
			Matrix<std::complex<C>> casted(m.getRows(), m.getCols());

			for (size_t i = 0;i < casted.getSize();++i) {
				casted(i).imag(static_cast<C>(m(i).imag()));
				casted(i).real(static_cast<C>(m(i).real()));
			}
			return casted;
		}
		...
		templ ColVector<T> conv2(noncmat mat1, cmat mat2, size_t stride, std::string shape)
		{
			...
			mat1.shed_row(0);
			mat1.shed_row(mat1.getRows()-1);
			mat1.shed_col(0);
			mat1.shed_col(mat1.getCols()-1);
		...
		{
			return tenseopr::dft(ColVector<std::complex<T>>(tenseopr::conv_to<std::complex<T>>(x)));
		}
		...
		templ ColVector<std::complex<double>> dft(cpvec x)
		{
			...
			RowVector<T> n = gen::regspace<T>(N);
			ColVector<T> k = n;

			Matrix<std::complex<double>> e(tenseopr::conv_to<std::complex<double>>(-2 * M_PI * tenseopr::conv_to<double>(k * n) / N));
			for (size_t i = 0;i < e.getSize();++i) {
				e(i) = pow(M_E, e(i));
			}

			return e * tenseopr::conv_to <double>(x);
		}
